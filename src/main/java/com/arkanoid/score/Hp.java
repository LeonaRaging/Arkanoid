...
public class Hp {
    ...
    public Hp(AnchorPane scene) {
        Image lifeImage = new Image(getClass().getResource("/com/arkanoid/Paddle/paddle00.png").toExternalForm());

        double startX = 200;
        double yPos = 74;
        double iconWidth = 16;
        double spacing = 8;
        double digitWidth = 10;
        double iconX = startX;
        double digitTensX = iconX + iconWidth + spacing;
        double digitUnitsX = digitTensX + spacing;
        ...
        showHp[0] = new Digit((int)digitTensX, (int)yPos, (int)digitWidth, 16, displayHp / 10); // Hàng chục
        showHp[1] = new Digit((int)digitUnitsX, (int)yPos, (int)digitWidth, 16, displayHp % 10);  // Hàng đơn vị
