...
public class Digit extends Entity {
    ...
    public Digit(int x, int y, int w, int h, int num) {
        super(x, y, w, h);
        image = new Image(getClass().getResource("/com/arkanoid/Number/" + Integer.toString(num) + ".png").toExternalForm());
    ...
    public void change(int newdigit){
        image = new Image(getClass().getResource("/com/arkanoid/Number/" + Integer.toString(newdigit) + ".png").toExternalForm());
        imageView.setImage(image);
        imageView.setX(this.getRectangle().getX());
        imageView.setY(this.getRectangle().getY());
    }
