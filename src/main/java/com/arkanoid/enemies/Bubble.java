...
public class Bubble extends Enemies {
  ...
  public boolean update(double deltaTime, AnchorPane scene) {
    switch (state) {
      ...
      case 6: // exist while catching
        ...
        if (remainingTime == 0) {
          ...
          ball.getCircle()
              .setLayoutX(this.getRectangle().getX() + this.getRectangle().getWidth() / 2);
          ball.getCircle()
              .setLayoutY(this.getRectangle().getY() + this.getRectangle().getHeight() / 2);
          ball.setDeltaX(-1);
          ball.setDeltaY(-1);
    ...
    imageView.setX(this.getRectangle().getX() + this.getRectangle().getWidth() / 2
        - images[imageDisplay][imageState].getWidth() / 2);
    imageView.setY(this.getRectangle().getY() + this.getRectangle().getHeight() / 2
        - images[imageDisplay][imageState].getHeight() / 2);
